...
interface FoodCardProps {
  ...
  onAddToCart: () => void; // ✅ ADDITION
}

interface FoodSectionProps {
  title: string;
  showMostPopular?: boolean;
  menus: any[]; // 👈 The menus are received here
...
export const FoodCard: React.FC<FoodCardProps> = ({
  ...
  const handleAddToCart = () => {
    onAddToCart(); // Correctly calls the function passed from FoodSection → MainContent → UserHomePage
  };
          ...
          {menus.map((menu, i) => (
            <div key={menu.menu_id} className="shrink-0">
              <FoodCard
                ...
                imageUrl={`http://localhost:8000/images/${menu.photo}`} // or a local path if the image is in /public
