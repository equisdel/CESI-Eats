...
async function getUser(role, mail) {
    try {
        let user = await User.findOne( { where: {mail: mail, role: role} })
        return user
    } catch (error) {
        return null
    }
...
const register = async (req, res) => {  // Changed to const definition
    const { role, mail, password, info } = req.body;
    const user = await getUser(role, mail);
    if (user) {
...
const login = async (req, res) => {  // Changed to const definition
    const { role, mail, password } = req.body;
    try {
        const user = await getUser(role, mail);
